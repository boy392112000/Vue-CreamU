    ...
    <div class="row">
      <div class="col-2">
        <div class="mb-3">
          ...
          <select v-model="selectedCity" @change="handleCityChange" class="form-select" required>
      ...
      <div class="col-2">
        <div class="mb-3">
          ...
          <select v-model="selectedDistrict" class="form-select" required>
            ...
            <option v-for="district in cityDistricts[selectedCity]" :key="district">{{ district }}
            </option>
          </select>
      ...
      <div class="col-8">
        <div class="mb-3"> <label class="form-label">地址 :</label>
...
const loadMember = async () => {
  ...
  city = member.value.address.substring(0, 3)
  region = member.value.address.substring(3, member.value.address.lastIndexOf("區") + 1)
  address = member.value.address.substring(member.value.address.lastIndexOf("區") + 1)
  console.log(city);
  console.log(region);
  console.log(address);
};
...
const cities = ["台北市", "新北市", "桃園市", "台中市", "台南市", "高雄市", "基隆市"
  , "新竹市", "嘉義市", "新竹縣", "苗栗縣", "彰化縣", "南投縣", "雲林縣", "嘉義縣", "屏東縣",
...
const cityDistricts = {
  ...
  新北市: [
    "板橋區", "三峽區", "淡水區", "金山區", "萬里區", "汐止區",
    "瑞芳區", "貢寮區", "平溪區", "雙溪區", "新店區", "深坑區",
  ],
  桃園市: [
    "桃園區", "中壢區", "平鎮區", "八德區", "楊梅區", "蘆竹區", "龜山區",
    "龍潭區", "復興區", "大溪區", "大園區", "觀音區", "新屋區"
  ],
  台中市: [
  ...
  高雄市: [
    "鹽埕區", "鼓山區", "左營區", "楠梓區", "三民區", "新興區", "前金區", "苓雅區", "那瑪夏區",
  ...
  新竹縣: [
    "竹北市", "竹東鎮", "新埔鎮", "關西鎮", "湖口鄉", "新豐鄉", "芎林鄉", "橫山鄉",
    "北埔鄉", "寶山鄉", "峨眉鄉", "尖石鄉", "五峰鄉"
  ],
  苗栗縣: [
    "苗栗市",
  ...
  彰化縣: [
    ...
    "埔鹽鄉", "埔心鄉", "大村鄉", "芳苑鄉", "竹塘鄉", "社頭鄉", "二水鄉", "田尾鄉",
    "埤頭鄉", "溪州鄉", "竹山鎮", "鹿草鄉", "水林鄉", "太保市", "中埔鄉", "竹崎鄉",
    "新港鄉", "民雄鄉", "大城鄉", "東勢鄉", "南投市", "後壁鄉", "造橋鄉", "潭子鄉",
    "北港鎮", "東區", "西區"
  ],
  南投縣: [
    "南投市",
...
async function handleSubmit() {
  try {
    const diet = {
      ...
      address: selectedCity.value + selectedDistrict.value + address,
